...
<p align="center">
  ...
  Web 面板 · OAuth 浏览器登录 · 账号池轮转 · 熔断与冷却 · 会话粘性 · 定时签到 / 活跃 / 旅行 / 保活 · <b>成长任务一键完成（17/18）</b> · <b>🌏 国际版账号（workbuddy.ai）</b> · 流式 / 非流式
</p>
...
> ### 🌏 本分支新增：国际版账号支持
>
> **本仓库是 [linguo2625469/workbuddy2api-panel](https://github.com/linguo2625469/workbuddy2api-panel) 的一个 fork，在原项目基础上增加了对 [WorkBuddy 国际站（`www.workbuddy.ai`）](https://www.workbuddy.ai) 账号的完整支持。**
>
> 国内站（`copilot.tencent.com`）与国际站是**同一套 `/v2/plugin/*` 协议的两套部署**，
> 本 fork 让两者可以**混挂同一个账号池**：凭据文件通过 `edition` 字段（`cn` / `intl`）标识站点，
> 网关按站点自动路由域名、Origin 与指纹头。
>
> | 新增能力 | 说明 |
> |---|---|
> | 🌏 **双站点账号池** | 国内站 / 国际站账号混挂轮询，凭据格式与 [CangShui/workbuddy-gateway](https://github.com/CangShui/workbuddy-gateway) 互通 |
> | 🖥️ **面板站点管理** | 账号表新增站点列 + 区域筛选；「添加账号」可选站点；「配置 → 上游站点」展示两站能力对照 |
> | 🎯 **按站点收窄请求** | `X-WB-Region: cn\|intl` 头，或 `model@intl` 后缀（便于无自定义头能力的 SDK） |
> | 🔀 **选号策略可切换** | `weighted`（默认，行为不变）/ `priority`（手动优先级）/ `round_robin`（轮流），面板可拖拽排序 |
> | 📋 **模型页站点切换 + 一键复制** | 国内站实时查询 / 国际站静态清单；模型名、API 地址、密钥均可一键复制 |
>
> **兼容性**：默认选号策略仍为 `weighted`，既有部署行为不变；无 `edition` 字段的老凭据自动视为国内站；
> 老 `config.json` / `state.json` 零改动兼容。
>
> 详细说明见 [🌏 国际版账号（workbuddy.ai）](#-国际版账号workbuddyai)。
>
> ---
>
> 原项目本身是 [Sliverkiss/workbuddy2api](https://github.com/Sliverkiss/workbuddy2api) 的增强分支（fork），
...
| 🔄 **多账号池** | 三因子加权随机选号（积分占比 ×10 + 闲置补偿 + 成功率 ×3），Top-5 候选 + 防惊群；**可切换手动优先级 / 轮流使用**，面板支持**拖拽 + ↑↓ 排序**，见 [选号策略与使用顺序](#选号策略与使用顺序) |
...
选号策略（`weighted` / `priority` / `round_robin`）与面板拖拽排序的完整说明见
[选号策略与使用顺序](#选号策略与使用顺序)；顺序同样只影响非粘性请求，两者关系见该节末尾的提示。

### 模型清单与一键复制

面板「**模型与档位**」页按站点分开展示，可切换：

- **国内站**：实时调用上游控制台接口（`source: "live"`）；
- **国际站**：国际站未挂载该接口，回退到内置实测静态清单（`source: "static"`，见[国际站实测可用模型](#国际站实测可用模型)）；
- 池中无可用国内站账号时，模型页给出可操作提示而非报错（模型名可自行填写，网关不校验白名单）。

为便于接入，面板提供三个一键复制按钮：

| 按钮 | 复制内容 |
|---|---|
| 模型名 | 单个模型 ID（列表中每行一个按钮） |
| API 请求地址 | `http://127.0.0.1:<端口>/v1`（端口按实际监听地址自动填充） |
| API 密钥 | 当前 `api_key`（配置页） |

> 复制走 `navigator.clipboard`，非 HTTPS / 非 localhost 场景自动回退 `execCommand`。
...
### 本 fork 新增（相对 [linguo2625469/workbuddy2api-panel](https://github.com/linguo2625469/workbuddy2api-panel) v1.6.3）

| 能力 | 说明 |
|---|---|
| 🌏 **国际版账号支持** | 国内站 / 国际站（`www.workbuddy.ai`）账号混挂同一池；凭据 `edition` 字段自动路由域名 / Origin / platform；面板按站点登录、站点列与区域筛选、能力置灰；`X-WB-Region` / `model@intl` 可按站点收窄请求。详见 [国际版账号](#-国际版账号workbuddyai) |
| 🔀 **账号使用顺序可调** | `pool.strategy`：`weighted`（默认，行为不变）/ `priority` / `round_robin`；面板「使用顺序」弹窗支持**拖拽 + ↑↓** 排序，顺序持久化于 `state.json` 的 `order` |
| 📋 **模型页站点切换** | 国内站实时查询 / 国际站静态清单可切换；模型名、`/v1` 地址、API 密钥均加**一键复制**按钮 |
| 🚫 **任务中心排除国际号** | 国际站成长任务接口 schema 不同（无进度/奖励字段），若不排除会产出 `task_code` 为空、进度为 0 的幽灵行并触发无效领奖调用；现已在服务端与面板两侧排除 |

> 默认策略仍为 `weighted`，无 `edition` 字段的老凭据自动视为国内站，老 `config.json` / `state.json` 零改动兼容。

### 原项目已含（相对上游 Sliverkiss/workbuddy2api）
...
**第三轮（`ea8b1e5` → `9a2adf2`，上游 v1.6.3，2026-09-14）**：

| 上游改动 | 吸收内容 |
|---|---|
| 积分展示 | **积分剩余 / 总额 + 自身百分比进度条**（`credits_total` 随 `state.json` 持久化，面板进度条按该号自身总额计算，替代原先无法判断"还剩多少"的绝对值显示） |
| 账户状态 | 账号表状态徽标与冷却显示同步上游最新口径 |
| 任务中心 | 上游新增能力原样保留（本 fork 在此基础上追加国际号排除逻辑，见上文） |
| 开学季 | 上游新增开学季排程小节原样保留 |

> 合并方式：真合并（`git merge`）而非覆盖，上游对本 fork 既有改动的冲突按**加法并集**解决。
> 其中出站 UA 冲突采用上游的官方三段式 `WorkBuddy/5.5.4 WorkBuddy/5.5.4 CLI/2.137.1` 作默认值，
> 站点 UA 降级为「仅显式配置时生效」。
...
> 💡 **本 README 描述的是带国际版支持的 fork**，克隆时请用本仓库地址
> `https://github.com/Yayi2/workbuddy2api-panel2.git`（原项目地址同样可用，但**不含**国际版功能）。

### 方式一：Docker Compose（推荐服务器部署）

```bash
# 1. 克隆（本 fork，含国际版账号支持）
git clone https://github.com/Yayi2/workbuddy2api-panel2.git
cd workbuddy2api-panel2
...
面板「使用顺序」弹窗内，顺序支持**拖拽排序**（按住行首手柄拖动）与 **↑↓ 按钮微调**两种方式，效果等价；
弹窗底部另有「恢复默认顺序」。

**排序不会改变账号的区域归属与可用性判定**：冷却/禁用/在途占满的账号会被自动跳过；
区域收窄（`X-WB-Region`）与顺序叠加生效。

> ⚠️ **会话粘性优先于选号策略**：开启 `session_sticky` 后，已有会话的请求直接按绑定的 uid 取号
> （`PickByUID` 命中即返回，**不经过策略分支**）。因此切换策略/调整顺序后，**旧会话仍用原账号**，
> 只有新会话才体现新顺序。若要让新顺序立刻作用于全部请求，需先清空会话绑定（或临时关闭粘性）。
