...
func (c *Client) ClaimReward(a *auth.Auth, taskCode string) (credit, energy int64, err error) {
	...
	req, err := http.NewRequest(http.MethodPost,
		web+"/activity/growth/tasks/"+url.PathEscape(taskCode)+"/claim", nil)
	if err != nil {
		return 0, 0, err
	}
	// Web 端请求头形状（对照浏览器实际请求）：Origin/Referer 指向该站成长中心，
	// 带 x-client-platform: web 标记来源端。域名必须与 webBase 同站，否则上游判跨站。
	req.Header.Set("Authorization", "Bearer "+a.AccessToken)
	req.Header.Set("Accept", "application/json, text/plain, */*")
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Origin", web)
	req.Header.Set("Referer", web+"/profile/growth-center")
